...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-xl font-bold text-slate-800">Contrats de catering</h2>
    <button onclick="document.getElementById('modalCreate').classList.remove('hidden')"
            class="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium shadow-sm transition">
        <i class="fa-solid fa-plus"></i> Nouveau contrat
    </button>
</div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr class="text-xs text-slate-400 uppercase">
                    <th class="px-5 py-3 text-left">Client / Entreprise</th>
                    <th class="px-5 py-3 text-left">Période</th>
                    <th class="px-5 py-3 text-right">Prix / repas</th>
                    <th class="px-5 py-3 text-center">Statut</th>
                    <th class="px-5 py-3 text-right">Repas planifiés</th>
                    <th class="px-5 py-3 text-right">Codes validés</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-50 hover:bg-slate-50 transition">
                    <td class="px-5 py-3">
                        <p class="font-semibold text-slate-800">{{ $c->client_name }}</p>
                        <p class="text-xs text-slate-400">{{ $c->company ?? '—' }}</p>
                    </td>
                    ...
                    <td class="px-5 py-3 text-right font-semibold text-teal-700">
                        {{ number_format($c->price_per_meal ?? 0, 0, ',', ' ') }} MRU
                    </td>
                    <td class="px-5 py-3 text-center">
                        @if($c->status === 'active')
                            <span class="px-2 py-1 rounded-full text-xs bg-emerald-100 text-emerald-700 font-medium">Actif</span>
                        @else
                            <span class="px-2 py-1 rounded-full text-xs bg-slate-100 text-slate-600 font-medium">Inactif</span>
                        @endif
                    </td>
                    <td class="px-5 py-3 text-right text-slate-600 font-medium">{{ $c->meals_count }}</td>
                    <td class="px-5 py-3 text-right">
                        <span class="px-2 py-1 rounded-full text-xs bg-teal-100 text-teal-700 font-medium">{{ $c->consumptions_count }}</span>
                    </td>
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <button onclick="openEditContract({{ $c->toJson() }})"
                                    class="w-8 h-8 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 inline-flex items-center justify-center transition"
                                    title="Modifier">
                                <i class="fa-solid fa-pen text-xs"></i>
                            </button>
                            @if($c->meals_count === 0)
                            <form method="POST" action="{{ route('catering.contracts.destroy', $c) }}"
                ...
                @empty
                <tr><td colspan="7" class="px-5 py-10 text-center text-slate-400">Aucun contrat — créez-en un !</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($contracts->hasPages())
    <div class="px-5 py-4 border-t border-slate-100">{{ $contracts->links() }}</div>
    @endif
</div>

{{-- CREATE MODAL --}}
<div id="modalCreate" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b">
            <h3 class="font-bold text-slate-800">Nouveau contrat</h3>
            <button onclick="document.getElementById('modalCreate').classList.add('hidden')" class="text-slate-400 hover:text-slate-600">
                <i class="fa-solid fa-xmark text-lg"></i>
            </button>
        </div>
        <form method="POST" action="{{ route('catering.contracts.store') }}" class="p-6 space-y-4">
            @csrf
            @include('catering._contract_form')
...
<div id="modalEdit" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b">
            <h3 class="font-bold text-slate-800">Modifier le contrat</h3>
        ...
        <form method="POST" id="editForm" class="p-6 space-y-4">
            @csrf @method('PUT')
            @include('catering._contract_form', ['edit' => true])
            <div class="flex justify-end gap-3 pt-2">
...
function openEditContract(c) {
    ...
    f.action = '/catering/contracts/' + c.id;
    f.querySelector('[name=client_name]').value   = c.client_name ?? '';
    f.querySelector('[name=company]').value        = c.company ?? '';
    f.querySelector('[name=start_date]').value     = c.start_date ? c.start_date.substring(0,10) : '';
    f.querySelector('[name=end_date]').value       = c.end_date   ? c.end_date.substring(0,10)   : '';
    f.querySelector('[name=price_per_meal]').value = c.price_per_meal ?? '';
    f.querySelector('[name=status]').value         = c.status ?? 'active';
